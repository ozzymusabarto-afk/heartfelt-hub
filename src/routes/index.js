...
import { Rocket, Target, BookOpen, Crown, BarChart3, Trophy, Settings, HelpCircle, CheckCircle2, Flame, Star, Shield, Search, Plus, ArrowRight, Check, Menu, X, Gamepad2, Dices, User, UserCheck, FileText, Undo2, ChevronDown, LogOut, ClipboardList, Timer, AlertCircle, ChevronLeft, Award, Download, Truck, Database, Info, Lightbulb, AlertTriangle } from "lucide-react";
import { XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, AreaChart, Area } from 'recharts';
import { MISSIONS as missions } from "@/data/missions";
import { CERTIFICATION_QUESTIONS } from "@/data/certificationQuestions";
import { PROFILE_TEST_QUESTIONS, PROFILE_METADATA } from "@/data/profileTestQuestions";
import { SAP_MODULES } from "@/data/sapModules";
...
const getRandomMissionIndex = (excludeIndex, reinforcementQueue = [], completedCount = 0) => {
...
const randomizeMissionData = (mission) => {
...
function HugoAvatar({ className }) {
    return (<div className={`rounded-2xl flex-shrink-0 flex items-center justify-center bg-gradient-to-tr from-indigo-600 to-blue-500 border-2 border-indigo-400 shadow-md relative group overflow-hidden ${className}`} role="img" aria-label="Avatar do Chefe Hugo">
      ...
      <div className="absolute -bottom-1 -right-1 size-4 bg-emerald-500 border-2 border-white dark:border-slate-800 rounded-full z-10 shadow-sm" aria-hidden="true"></div>
    </div>);
}
function ProfileTestModule() {
    ...
    if (testState === "intro") {
        return (<Card className="flex-1 p-8 flex flex-col items-center justify-center text-center bg-white border-slate-200 rounded-3xl animate-in fade-in zoom-in-95 duration-300">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full max-w-2xl mb-10">
          {["Consultor", "Key User", "Suporte", "Arquiteto"].map((p) => (<div key={p} className="p-3 bg-slate-50 rounded-2xl border border-slate-100">
        ...
        <Button onClick={startTest} className="h-14 px-10 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl shadow-lg shadow-indigo-100 gap-2 text-lg transition-all active:scale-95">
    ...
    if (testState === "testing" && currentQuestion) {
        ...
        return (<div className="flex-1 flex flex-col gap-4 animate-in fade-in duration-300">
        ...
        <Card className="flex-1 p-8 bg-white border-slate-200 rounded-3xl shadow-sm flex flex-col">
          ...
          <div className="grid grid-cols-1 gap-3 flex-1">
            {currentQuestion.options.map((option) => (<button key={option.id} onClick={() => handleAnswer(option.profile)} className="w-full p-5 border-2 border-slate-100 hover:border-indigo-600 hover:bg-indigo-50 rounded-2xl text-left transition-all flex items-start gap-4 group">
    ...
    if (testState === "result") {
        ...
        return (<div className="flex-1 flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-300">
        <Card className="p-8 flex flex-col items-center bg-white border-slate-200 rounded-3xl shadow-sm overflow-hidden relative">
          ...
          <div className="w-full max-w-3xl grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {results.map((res) => (<div key={res.profile} className="space-y-1.5">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-2">
                {metadata.strengths.map((s, i) => (<div key={i} className="flex items-center gap-2 p-2 bg-emerald-50/50 rounded-xl border border-emerald-100/50">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-2">
                {metadata.developmentPoints.map((dp, i) => (<div key={i} className="flex items-center gap-2 p-2 bg-indigo-50/50 rounded-xl border border-indigo-100/50">
          ...
          <div className="w-full max-w-3xl bg-slate-50 rounded-3xl border border-slate-100 p-8 mb-8">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {metadata.transactions.map(t => (<Badge key={t} variant="outline" className="bg-white border-slate-200 text-slate-600 font-mono text-[10px] px-2 py-0.5 rounded-lg shadow-sm">
                  {t}
                </Badge>))}
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Button onClick={() => setTestState("intro")} variant="outline" className="border-slate-200 text-slate-500 font-bold rounded-xl h-12 px-6">
              REFAZER TESTE
            </Button>
            <Button onClick={() => {
...
function CertificationModule() {
    ...
    if (examState === "intro") {
        ...
        <Button onClick={() => setExamState("mode-select")} className="h-14 px-10 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl shadow-lg shadow-indigo-100 gap-2 text-lg transition-all active:scale-95">
          INICIAR SIMULADO <ArrowRight className="size-5"/>
        </Button>
      </Card>);
    }
    if (examState === "mode-select") {
        return (<Card className="flex-1 p-8 flex flex-col items-center justify-center bg-white border-slate-200 rounded-3xl animate-in fade-in zoom-in-95 duration-300">
        <h2 className="text-2xl font-black text-slate-800 mb-8 tracking-tight">Escolha o Modo de Simulação</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-2xl">
          <button onClick={() => startExam("study")} className="p-8 border-2 border-slate-100 hover:border-indigo-600 hover:bg-indigo-50 rounded-3xl text-left transition-all group">
          ...
          <button onClick={() => startExam("exam")} className="p-8 border-2 border-slate-100 hover:border-amber-500 hover:bg-amber-50 rounded-3xl text-left transition-all group">
        ...
        <Card className="flex-1 p-8 bg-white border-slate-200 rounded-3xl shadow-sm flex flex-col">
          ...
          <div className="space-y-3 flex-1">
            {currentQuestion.options.map((option) => {
                ...
                return (<button key={option.id} disabled={isAnswered && examMode === "study"} onClick={() => handleAnswer(option.id)} className={`w-full p-4 border-2 ${borderColor} ${bgColor} rounded-2xl text-left transition-all flex items-start gap-4 group disabled:cursor-default`}>
          ...
          {showExplanation && (<div className="mt-8 p-6 bg-slate-50 rounded-2xl border border-slate-200 animate-in slide-in-from-bottom-2 duration-300">
          ...
          <div className="mt-8 pt-8 border-t border-slate-100 flex justify-end">
            <Button disabled={!isAnswered} onClick={nextQuestion} className="h-12 px-8 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl gap-2 shadow-lg shadow-indigo-100">
    ...
    if (examState === "result") {
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full max-w-xl">
          <Button className="flex-1 h-12 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg shadow-indigo-100" onClick={() => setExamState("mode-select")}>
            TENTAR NOVAMENTE
          </Button>
          <Button variant="outline" className="flex-1 h-12 border-slate-200 text-slate-600 font-bold rounded-xl" onClick={() => setExamState("intro")}>
            VOLTAR AO INÍCIO
          </Button>
        </div>
        ...
        <div className="mt-12 w-full max-w-2xl">
          ...
          <div className="space-y-4">
            {examQuestions.map((q, idx) => (<Card key={q.id} className="p-6 border-slate-100 bg-slate-50/50 rounded-2xl">
...
function CertificateModule({ completedMissions, xp, certName, setCertName, certificateRef }) {
    ...
    if (!showCertificate) {
        ...
        {!isEligible ? (<div className="text-center max-w-md">
            ...
            <div className="space-y-2">
              ...
              <Input value={certName} onChange={(e) => setCertName(e.target.value)} placeholder="Ex: João da Silva Santos" className="h-12 rounded-xl border-slate-200 focus:ring-indigo-500"/>
            </div>

            <Button disabled={!certName.trim()} onClick={() => setShowCertificate(true)} className="w-full h-14 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl shadow-lg shadow-indigo-100 gap-2 text-lg transition-all active:scale-95">
    ...
    return (<div className="flex-1 flex flex-col gap-6 animate-in fade-in duration-300">
      <div className="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
        <Button variant="ghost" onClick={() => setShowCertificate(false)} className="gap-2 text-slate-500 font-bold">
          <ChevronLeft className="size-4"/> VOLTAR
        </Button>
        <div className="flex gap-3">
          <Button onClick={handleDownload} disabled={isGenerating} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl gap-2 shadow-md shadow-indigo-100">
      ...
      <div className="overflow-x-auto pb-8">
        <div ref={certificateRef} className="min-w-[1000px] aspect-[1.414/1] bg-white border-[16px] border-double border-indigo-900 p-16 flex flex-col items-center justify-between text-center shadow-2xl mx-auto relative overflow-hidden" style={{ fontFamily: "'Fraunces', serif" }}>
...
function SAPSDQuestApp() {
    ...
    const [reinforcementQueue, setReinforcementQueue] = useState([]);
    ...
    const [f1ActiveField, setF1ActiveField] = useState(null);
    const seniorityLevel = useMemo(() => {
        if (completedMissions >= 131)
            return "Sênior";
        if (completedMissions >= 81)
            return "Pleno";
        if (completedMissions >= 41)
            return "Júnior";
        return "Trainee";
    }, [completedMissions]);
        ...
        const handleStorageChange = (e) => {
            if (e.key === "sap-quest-data" && e.newValue) {
                try {
                    ...
                    if (parsed.feedbackState && parsed.feedbackState !== feedbackState)
                        setFeedbackState(parsed.feedbackState);
                }
                catch (err) {
    ...
    const successRate = filteredHistory.length > 0
        ...
        : 0;
    if (!isAuth) {
        return (<div className="min-h-screen bg-[#f4f7fc] flex flex-col items-center justify-center p-4 font-sans">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input value={userName} onChange={(e) => setUserName(e.target.value)} placeholder="Seu nome" className="h-12 pl-10 rounded-xl border-slate-200 focus:ring-indigo-600 bg-slate-50/50"/>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="h-12 pl-10 rounded-xl border-slate-200 focus:ring-indigo-600 bg-slate-50/50"/>
              </div>
            </div>
    ...
    return (<div className="min-h-screen bg-background text-foreground font-sans flex flex-col pb-20 md:pb-0 overflow-y-auto">
      {showOnboarding && (<div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-300">
          <Card className="w-full max-w-[500px] border-none shadow-2xl rounded-3xl overflow-hidden bg-white animate-in zoom-in-95 duration-300">
            <div className="h-2 w-full bg-slate-100">
              <div className="h-full bg-indigo-600 transition-all duration-500" style={{ width: `${((onboardingStep + 1) / 3) * 100}%` }}/>
            </div>
            
            <div className="p-8">
              {onboardingStep === 0 && (<div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
              ...
              {onboardingStep === 1 && (<div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
              ...
              {onboardingStep === 2 && (<div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
              ...
              <div className="flex items-center gap-3 mt-10">
                <Button variant="ghost" className="font-bold text-slate-400" onClick={finishOnboarding}>
                  PULAR
                </Button>
                <div className="flex-1"/>
                {onboardingStep < 2 ? (<Button className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-8 rounded-xl" onClick={() => setOnboardingStep(onboardingStep + 1)}>
                    AVANÇAR
                  </Button>) : (<Button className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-8 rounded-xl shadow-lg shadow-indigo-100" onClick={finishOnboarding}>
      ...
      {showWelcomeModal && (<div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-300">
          <Card className="w-full max-w-[550px] border-none shadow-2xl rounded-3xl overflow-hidden bg-white animate-in zoom-in-95 duration-300">
            ...
            <div className="p-8">
            ...
            ].map((step, i) => (<li key={i} className="flex gap-3 text-xs text-slate-600">
              ...
              <Button className="w-full h-14 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl shadow-lg shadow-indigo-100 text-lg transition-all active:scale-[0.98]" onClick={() => {
      ...
      <header className="sticky top-0 z-50 w-full border-b border-border bg-card shadow-sm px-4 md:px-6 py-3 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 border-l ml-3 pl-3 border-slate-200">
            <Button variant="ghost" size="icon" className="size-9 rounded-xl text-slate-400 hover:text-red-500 hover:bg-red-50 transition-colors" onClick={handleLogoutClick} title="Sair do Sistema">
      ...
      <div className="flex-1 flex flex-col md:flex-row relative">
        <aside className={`fixed inset-y-0 left-0 z-40 w-[260px] bg-card border-r p-6 transform transition-transform md:relative md:translate-x-0 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} h-screen md:h-auto flex flex-col`}>
          <nav className="space-y-1 flex-1">
            ...
            if (item.adminOnly && !isUserAdmin)
                return null;
            const Content = (<Button key={item.label} variant="ghost" className={`w-full justify-start h-12 px-3 py-2 rounded-xl gap-3 ${activeTab === item.id ? "bg-indigo-600 text-white shadow-md shadow-indigo-100" : "text-slate-500 hover:bg-indigo-50 hover:text-indigo-600"}`} onClick={() => {
                    if (item.premium) {
                  ...
                  <div className="relative">
                    ...
                    {item.premium && (<div className="absolute -top-1.5 -right-1.5 bg-amber-500 rounded-full border border-white p-0.5 shadow-sm">
            ...
            if (item.path) {
                return (<Link to={item.path} key={item.label} className="block no-underline">
                    {Content}
                  </Link>);
            }
            return Content;
        ...
        <main className="bg-slate-50 p-4 flex-1 flex flex-col gap-4">
          {activeTab === "certificate" && (<CertificateModule completedMissions={completedMissions} xp={xp} certName={certName} setCertName={setCertName} certificateRef={certificateRef}/>)}
          {activeTab === "certificate" && (<CertificateModule completedMissions={completedMissions} xp={xp} certName={certName} setCertName={setCertName} certificateRef={certificateRef}/>)}
          {activeTab === "main" && (<>
          ...
          <Card className="p-0 border-slate-200 shadow-sm rounded-2xl overflow-hidden shrink-0">
            <div className="p-4 flex items-center justify-between gap-4 bg-white">
              ...
              <div className="relative">
                <Button variant="outline" size="sm" className={`h-8 text-[10px] font-bold gap-1.5 border-slate-200 rounded-lg transition-colors ${isHelpOpen ? 'bg-indigo-600 text-white border-indigo-600' : 'text-slate-500'}`} onClick={() => setIsHelpOpen(!isHelpOpen)}>
                  <HelpCircle className="size-3"/> AJUDA DO CAMPO [F1]
                </Button>
                
                {isHelpOpen && (<Card ref={helpContainerRef} className="fixed inset-0 md:absolute md:inset-auto md:right-0 md:top-10 w-full h-full md:w-[320px] md:h-auto md:max-h-[80vh] p-0 z-[60] shadow-2xl border-indigo-100 animate-in fade-in slide-in-from-bottom-4 md:slide-in-from-top-2 duration-200 flex flex-col bg-white overflow-hidden rounded-none md:rounded-2xl" role="dialog" aria-modal="true" aria-labelledby="help-title">
                    <div className="flex justify-between items-center p-5 border-b border-slate-100 bg-white sticky top-0 z-10">
                      ...
                      <Button variant="ghost" size="icon" className="size-8 h-8 w-8 hover:bg-slate-100 rounded-lg ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-600" onClick={() => setIsHelpOpen(false)} ref={helpCloseRef} aria-label="Fechar ajuda">
                        <X className="size-5 text-slate-400"/>
                      </Button>
                    </div>

                    <div ref={helpScrollRef} className="flex-1 overflow-y-auto p-5 space-y-3 custom-scrollbar bg-slate-50/30">
                      {[
                    {
                        id: "MISSION_OBJ",
                    ...
                    },
                    {
                        ...
                        variant: "slate"
                ...
                ].map((section) => (<div key={section.id} className="border border-slate-200 rounded-xl overflow-hidden bg-white shadow-sm">
                          <button onClick={() => toggleSection(section.id)} className="w-full flex items-center justify-between p-3 text-left hover:bg-slate-50 transition-colors">
                            <div className="flex items-center gap-2">
                          ...
                          {expandedSections[section.id] && (<div className="p-3 pt-0 text-[11px] text-slate-500 leading-relaxed animate-in fade-in slide-in-from-top-1 duration-200">
            ...
            ].map((step, i) => (<div key={i} className="flex items-center gap-2 flex-1 group">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card className="p-4 flex flex-col bg-white border-slate-200 shadow-sm rounded-2xl">
              ...
              <RadioGroup value={selectedTransaction} onValueChange={setSelectedTransaction} className="space-y-2 pr-1">
                ...
                if (!id)
                    return null;
                return (<Label key={id} className={`flex items-center gap-2 p-3 border rounded-xl cursor-pointer transition-all ${selectedTransaction === id ? "border-indigo-600 bg-indigo-50/50 ring-1 ring-indigo-600" : "border-slate-100 hover:bg-slate-50"}`}>
            ...
            <Card className={`p-4 bg-white border-slate-200 shadow-sm rounded-2xl flex flex-col ${!selectedTransaction ? "opacity-50 pointer-events-none" : ""}`}>
              ...
              <div className="grid grid-cols-2 gap-3 pr-1">
                ...
                return fieldsToShow.map((field) => {
                    ...
                    return (<div key={field.id} className="space-y-1">
                        <div className="flex items-center justify-between">
                          <Label className="text-[9px] font-black text-slate-400 uppercase tracking-wider">{field.label}</Label>
                          <Button variant="ghost" size="icon" className="size-4 h-4 w-4 p-0 text-slate-300 hover:text-indigo-600 transition-colors" onClick={() => openF1ForField(field.id)} title="Ajuda F1">
                            <HelpCircle className="size-2.5"/>
                          </Button>
                        </div>
                        {field.type === "select" ? (<Select value={formData[fieldId] || ""} onValueChange={(v) => handleInputChange(field.id, v)}>
                          ...
                          </Select>) : (<div className="relative group">
                            <Input value={formData[fieldId] || ""} onChange={(e) => handleInputChange(field.id, e.target.value)} className={`h-9 rounded-lg border-slate-200 text-xs placeholder:text-slate-300 focus:ring-indigo-600 ${field.hasSearch ? "pr-8" : ""} ${validationErrors.includes(field.id) ? "border-red-400 ring-1 ring-red-400" : ""}`}/>
                            {field.hasSearch && (<Button size="icon" variant="ghost" className="absolute right-0 top-0 h-9 w-8 text-slate-400 hover:text-indigo-600" onClick={() => setIsCustomerSearchOpen(true)} title="Busca F4 (Matchcode)">
              ...
              {(selectedTransaction === "VA05" || selectedTransaction === "V.02") && (<div className="mt-4 pt-4 border-t border-slate-100">
                  <Button className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl h-10 gap-2 shadow-md shadow-indigo-100" onClick={() => {
              ...
              <Button onClick={handleSubmit} className="w-full h-10 mt-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg shadow-indigo-200 transition-all active:scale-[0.98]">
                📝 CONFERIR E SUBMETER
              </Button>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <Card className="p-4 bg-white shadow-sm border-indigo-100 rounded-2xl border-l-4 border-l-indigo-600 relative overflow-hidden group">
                <div className="relative z-10 space-y-3">
                  ...
                  <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-indigo-600 transition-all duration-1000 ease-out" style={{ width: `${(completedMissions / missions.length) * 100}%` }}/>
                  </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="grid grid-cols-1 gap-2">
                <Card className="p-3 bg-white border-indigo-600 shadow-sm rounded-xl flex items-center justify-between ring-1 ring-indigo-600 group hover:bg-indigo-50/10 transition-colors">
                  <div className="flex items-center gap-3">
          ...
          {activeTab === "quick" && (<Card className="flex-1 p-8 flex flex-col items-center justify-center text-center bg-white border-slate-200 rounded-3xl animate-in fade-in zoom-in-95 duration-300">
              <div className="size-20 bg-indigo-100 rounded-3xl flex items-center justify-center text-indigo-600 mb-6 shadow-xl shadow-indigo-50">
                <Rocket className="size-10"/>
              </div>
          ...
          {activeTab === "docs" && (<div className="flex-1 flex flex-col gap-6 animate-in fade-in duration-300">
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                ...
                <div className="relative w-full md:w-72">
                  ...
                  <Input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Buscar T-Code ou Tópico..." className="pl-9 h-10 rounded-xl border-slate-200 focus:ring-indigo-500 bg-white"/>
                </div>
              </div>

              {selectedModule ? (<div className="animate-in fade-in slide-in-from-right-4 duration-300 space-y-6 pb-20">
                  <Button variant="ghost" onClick={() => setSelectedModule(null)} className="gap-2 text-slate-500 font-bold hover:text-indigo-600 pl-0">
                  ...
                  <Card className="overflow-hidden bg-white border-slate-200 rounded-3xl shadow-sm">
                    ...
                    <div className="p-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
                      <div className="space-y-8">
                        <div>
                          ...
                          <div className="grid grid-cols-1 gap-2">
                            {selectedModule.transactions.map((t, idx) => (<div key={idx} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100 group hover:border-indigo-200 transition-colors">
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-1 gap-2">
                            {selectedModule.tables.map((t, idx) => (<div key={idx} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100 group hover:border-indigo-200 transition-colors">
                      ...
                      <div className="space-y-8">
                        <div>
                          ...
                          <div className="space-y-3">
                            {selectedModule.tips.map((tip, idx) => (<div key={idx} className="flex gap-3 p-4 bg-amber-50/50 rounded-2xl border border-amber-100/50">
                        ...
                        <div>
                          ...
                          <div className="space-y-3">
                            {selectedModule.commonErrors.map((error, idx) => (<div key={idx} className="flex gap-3 p-4 bg-red-50/50 rounded-2xl border border-red-100/50">
                    ...
                    <div className="bg-slate-50 p-6 border-t border-slate-100 flex items-center justify-between">
                      ...
                      <Button onClick={() => setSelectedModule(null)} className="bg-indigo-600 text-white font-bold rounded-xl h-10 px-6">
                        ENTENDI, VOLTAR
                      </Button>
                    </div>
                  </Card>
                </div>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {SAP_MODULES.filter(mod => mod.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
                    mod.summary.toLowerCase().includes(searchQuery.toLowerCase()) ||
                    mod.transactions.some(t => t.tcode.toLowerCase().includes(searchQuery.toLowerCase()))).map((mod, i) => (<Card key={mod.id} onClick={() => setSelectedModule(mod)} className="p-6 bg-white border-slate-200 hover:border-indigo-600 hover:shadow-xl hover:shadow-indigo-50 transition-all rounded-3xl group cursor-pointer flex flex-col h-full relative overflow-hidden">
                      ...
                      <div className="flex flex-wrap gap-1.5 mb-6">
                        {mod.transactions.slice(0, 3).map((t, ti) => (<Badge key={ti} variant="outline" className="bg-slate-50 text-[9px] font-mono font-bold text-slate-400 border-slate-100">
                            {t.tcode}
                          </Badge>))}
          ...
          {activeTab === "stats" && (<div className="flex-1 flex flex-col gap-6 animate-in fade-in duration-300">
              <div className="flex flex-col gap-1">
            ...
            ].map((stat, i) => (<Card key={i} className="p-4 bg-white border-slate-200 rounded-2xl flex items-center gap-4">
          ...
          {activeTab === "achievements" && (<div className="flex-1 flex flex-col gap-6 animate-in fade-in duration-300">
              <div className="flex flex-col gap-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                { title: "Pronto para o Projeto", desc: "Concluir todas as 170 missões", icon: Trophy, unlocked: completedMissions >= 170 }
            ].map((ach, i) => (<Card key={i} className={`p-6 flex flex-col items-center text-center rounded-2xl border-2 transition-all ${ach.unlocked ? "bg-white border-indigo-100 shadow-lg shadow-indigo-50" : "bg-slate-50 border-slate-100 opacity-60 grayscale"}`}>
                    ...
                    {ach.unlocked ? (<Badge className="mt-4 bg-emerald-500 text-white border-none">DESBLOQUEADO</Badge>) : (<Badge variant="outline" className="mt-4 bg-transparent text-slate-400 border-slate-200">BLOQUEADO</Badge>)}
                  </Card>))}
              </div>
            </div>)}

          {activeTab === "settings" && (<div className="flex-1 flex flex-col gap-6 animate-in fade-in duration-300 max-w-2xl mx-auto w-full">
              <div className="flex flex-col gap-1">
        ...
        <aside className="hidden lg:flex w-[320px] border-l bg-card p-5 flex-col gap-6">
          <Card className={`p-4 border shadow-sm rounded-2xl flex flex-col items-center text-center relative overflow-hidden ${feedbackState === "success" ? "bg-emerald-50 border-emerald-200" : feedbackState === "error" ? "bg-red-50 border-red-200" : "bg-white border-slate-200"}`}>
            ...
            {feedbackState === "success" && (<div className="w-full space-y-2">
                <div className="bg-emerald-100 text-emerald-700 font-black text-xs py-2 rounded-xl mb-2">+25 XP</div>
                <Button onClick={nextMission} className="w-full text-xs h-10 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl gap-2 shadow-lg shadow-emerald-100">
                  PRÓXIMO PEDIDO <ArrowRight className="size-4"/>
                </Button>
                <Button variant="outline" onClick={generatePDFReport} className="w-full h-10 border-indigo-200 text-indigo-600 hover:bg-indigo-50 font-bold rounded-xl flex items-center justify-center gap-2 animate-in fade-in zoom-in-95 duration-300 text-xs">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between pl-1">
                ...
                <div className="relative">
                  <Input placeholder="Buscar..." value={historySearch} onChange={(e) => setHistorySearch(e.target.value)} className="h-6 w-24 text-[9px] bg-slate-50 border-none rounded-md"/>
                </div>
              </div>
              <Card className="bg-white border-slate-200 shadow-sm rounded-2xl overflow-hidden">
                <div className="max-h-[180px] overflow-y-auto divide-y divide-slate-50">
                  {filteredHistory.length === 0 ? (<div className="p-4 text-center text-[10px] text-slate-400 italic">
                      {historySearch ? "Nenhum resultado." : "Nenhuma submissão."}
                    </div>) : (filteredHistory.map((h) => (<div key={h.id} className="p-3 hover:bg-slate-50 transition-colors">
                        ...
                        {h.status === 'success' && (<div className="flex items-center justify-between mt-1 text-[8px] text-slate-500 font-semibold">
            ...
            <div className="space-y-2">
              {showUndoReset && (<Button variant="default" size="sm" className="w-full h-8 text-[10px] font-bold bg-amber-500 hover:bg-amber-600 text-white rounded-xl gap-2 animate-bounce" onClick={undoReset}>
                  <Undo2 className="size-3"/> DESFAZER REINÍCIO
                </Button>)}
              <Button variant="outline" size="sm" className="w-full h-8 text-[10px] font-bold text-red-500 hover:text-red-600 hover:bg-red-50 border-red-100 rounded-xl gap-2 transition-all active:scale-[0.98]" onClick={fullReset}>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between pl-1">
              ...
              <Button variant="ghost" size="sm" className="h-6 px-2 text-[9px] font-bold text-indigo-600 hover:bg-indigo-50 gap-1 rounded-md" onClick={generatePDFReport}>
            ...
            total: trainingHistory.filter(hs => hs.status === 'success').reverse().slice(0, i + 1).reduce((acc, curr) => acc + (curr.xpEarned || 0), 0)
                    ...
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9"/>
                    <Tooltip contentStyle={{ fontSize: '10px', borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} labelStyle={{ display: 'none' }}/>
            ...
            "Dashboard avançado"
        ].map((item, i) => (<li key={i} className="flex items-center gap-2 text-[10px] text-indigo-100 font-medium">
      ...
      {isCustomerSearchOpen && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
            ...
            <div className="p-0 max-h-[400px] overflow-y-auto">
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                  {CUSTOMER_MASTER.map((c) => (<tr key={c.code} className="hover:bg-indigo-50 cursor-pointer transition-colors group" onClick={() => {
      ...
      {isLogoutModalOpen && (<div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-300">
          <Card className="w-full max-w-[450px] border-none shadow-2xl rounded-3xl overflow-hidden bg-white animate-in zoom-in-95 duration-300">
            ...
            <div className="p-8 flex flex-col items-center text-center">
              ...
              <div className="grid grid-cols-2 gap-3 w-full">
                <Button variant="outline" className="h-12 rounded-xl font-bold border-slate-200 text-slate-500 hover:bg-slate-50" onClick={() => setIsLogoutModalOpen(false)}>
                  CANCELAR
                </Button>
                <Button className="h-12 rounded-xl font-bold bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-100" onClick={confirmLogout}>
                  CONFIRMAR SAÍDA
                </Button>
              </div>
            </div>
      ...
      {/* F1 HELP MODAL */}
      {isF1ModalOpen && f1ActiveField && (<div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[110] flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={() => setIsF1ModalOpen(false)}>
          <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="f1-modal-title">
            {/* Modal Header */}
            <div className="bg-indigo-600 px-6 py-4 flex items-center justify-between text-white">
              ...
              <button onClick={() => setIsF1ModalOpen(false)} className="p-2 hover:bg-white/10 rounded-full transition-colors outline-none focus:ring-2 focus:ring-white/50" aria-label="Fechar modal">
                <X className="size-6"/>
              </button>
            </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-800/50 px-6 py-4 flex justify-end border-t border-slate-100 dark:border-slate-800">
              <Button onClick={() => setIsF1ModalOpen(false)} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-8 rounded-lg">
                ENTENDI
              </Button>
            </div>
          </div>
