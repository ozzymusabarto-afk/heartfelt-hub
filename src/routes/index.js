...
import { Rocket, Target, BookOpen, Crown, BarChart3, Trophy, Settings, HelpCircle, CheckCircle2, Star, Shield, Search, Plus, ArrowRight, Check, Menu, X, Gamepad2, Dices, User, UserCheck, FileText, Undo2, ChevronDown, LogOut, ClipboardList, Timer, AlertCircle, ChevronLeft, Award } from "lucide-react";
        ...
        <Card className="p-8 flex flex-col items-center bg-white border-slate-200 rounded-3xl shadow-sm overflow-hidden relative">
          ...
          <div className="w-full max-w-2xl grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-2">
                {metadata.strengths.map((s, i) => (<div key={i} className="flex items-center gap-2 p-2 bg-emerald-50/50 rounded-lg border border-emerald-100/50">
          ...
          <div className="w-full max-w-2xl bg-slate-50 rounded-2xl border border-slate-100 p-6 mb-8">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {metadata.transactions.map(t => (<Badge key={t} variant="outline" className="bg-white border-slate-200 text-slate-600 font-mono text-[10px] px-2">
                  {t}
                </Badge>))}
            </div>
          </div>

          <Button onClick={() => setTestState("intro")} variant="outline" className="border-slate-200 text-slate-500 font-bold rounded-xl">
            REFAZER TESTE
          </Button>
        </Card>
      </div>);
